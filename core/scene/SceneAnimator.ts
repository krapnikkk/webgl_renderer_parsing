...
export default class SceneAnimator{
    scene: any;
    animations: any[];
    skinningRigs: any[];
    meshIDs: any[];
    ...
    views: any[];
    viewYawOffsets: any[];
    viewPitchOffsets: any[];
    cameraObjectIndices: any[];
    cameraChildrenIndices: any[];
    subMeshObjectIndices: any[];
    subMeshLiveIndices: any[];
    selectedCameraIndex: number;
    selectedAnimationIndex: number;
    debugString: string;
    scenePlaybackSpeed: number;
    playbackSpeed: number;
    animationProgress: number;
    totalSeconds: number;
    paused: boolean;
    autoAdvanceNextAnimation: boolean;
    animateVisibility: boolean;
    drawAnimated: boolean;
    linkSceneObjects: boolean;
    loadSkinningRigs: boolean;
    animateMaterials: boolean;
    animateTurntables: boolean;
    ...
    loopCount: number;
    loopTurntables: boolean;
    lockPlayback: boolean;
    fogObjectIndex: number;
    unitScaleSkinnedMeshes: boolean;
    sceneScale: any;
    defaultCameraGlobalIndex: any;
    autoPlayAnims: any;
    showPlayControls: any;
    numMatricesInTable: any;
    startMS: number;
    stopEverything: boolean;
    runDebugMode: boolean;
    lastSceneFramePercent: number;
    constructor(a, c, b) {
        this.scene = a;
        this.animations = [];
        this.skinningRigs = [];
        ...
        this.paused = this.autoAdvanceNextAnimation = false;
        this.animateVisibility = this.drawAnimated = this.linkSceneObjects = this.loadSkinningRigs = this.animateMaterials = this.animateTurntables = this.enableSkinning = this.animateMeshes = this.animateLights = this.playAnimations = this.loadAnimations = true;
        this.showDebugInfo = false;
        this.loopCount = 0;
        this.loopTurntables = this.lockPlayback = false;
        this.fogObjectIndex = -1;
        this.unitScaleSkinnedMeshes = true;
        ...
        this.autoPlayAnims || (this.paused = true);
        if (this.loadAnimations) {
            ...
            if (this.linkSceneObjects && 0 != f) {
                ...
                this.showDebugInfo = this.stopEverything = this.runDebugMode = false;
                this.selectDefaultCamera();
                this.findCameraChildren();
                this.findFixedTransforms();
                this.runDebugMode && (this.setAnimationProgress(0, true),
                    "" != this.debugString ? this.stopEverything = true : this.checkDebug())
            }
        } else
    ...
    flagAllForDebugging() {
        for (var a = this.animations.length, c = 0; c < a; c++)
            for (var b = this.animations[c], d = b.animatedObjects.length, e = 0; e < d; e++)
                b.animatedObjects[e].debugMe = true
    }
        ;
    checkDebug() {
        ...
        "" != a.debugString && (this.debugString += "<br>--------------------------------------------------Got animation bug info:",
            ...
            this.showDebugInfo = this.stopEverything = true,
            a.debugString = "");
        for (var b = 0; b < c; b++) {
            var d = a.animatedObjects[b];
    ...
    logObjectInfo(a, c) {
        ...
        if (a >= b.animatedObjects.length)
            ...
            d = Matrix.identity();
            b.getWorldTransform(e.id, c, d, this.sceneScale, true);
    ...
    resetPlayback() {
        this.startMS = Date.now();
        this.animationProgress = this.totalSeconds = 0;
        this.setAnimationProgress(0, true)
    }
        ;
    pause(a) {
        this.paused = a;
        ...
        if (0 <= this.selectedCameraIndex && this.selectedCameraIndex < this.views.length && this.selectedAnimationIndex < this.animations.length) {
            ...
            if (a < c.animatedObjects.length) {
                ...
                var f = Matrix.identity()
                    , g = Matrix.identity();
                b.useFixedWorldTransform = false;
                b.useFixedLocalTransform = false;
                b.animatedLocalTransform.lockTransform = false;
                ...
                b.cachedFrameUse2 = 0;
                b.cachedFrameUse3 = 0;
                c.getWorldTransform(a, this.totalSeconds, g, this.sceneScale, false);
                var a = g[0]
                    , h = g[1]
                    , k = g[2]
                    , n = g[4]
                ...
                f[14] = g;
                e = Matrix.identity();
                c.getWorldTransform(b.parentIndex, this.totalSeconds, e, this.sceneScale, false);
                c = Matrix.identity();
                Matrix.invert(c, e);
                e = Matrix.identity();
                Matrix.mul(e, c, f);
                ...
                if (!f.useFixedWorldTransform && !b.hasAnimationInHierarchy(f))
                    if ("Material" == f.sceneObjectType)
                        ...
                        var g = Matrix.identity()
                            , h = Matrix.identity();
                        b.hasParentTypeInHierarchy(f, "SceneRootSO") ? (b.getWorldTransform(f.id, 0, g, this.sceneScale, false),
                            f.evaluateLocalTransformAtFramePercent(0, h, true, false)) : (b.evaluateModelPartTransformAtFrame(f.id, 0, g, false),
                                f.evaluateLocalTransformAtFramePercent(0, h, false, false));
    ...
    clearCacheForCameraChildren() {
        for (var a = this.animations[this.selectedAnimationIndex], c = this.cameraChildrenIndices.length, b = 0; b < c; b++) {
            ...
            d.useFixedWorldTransform = false;
            d.useFixedLocalTransform = false;
            d.cachedFrame0 = -10;
            d.cachedFrame1 = -10;
            d.cachedFrame2 = -10;
            d.cachedFrame3 = -10;
            ...
            d.animatedLocalTransform.lockTransform = false
        }
    }
        ;
    refreshTransformsOnly() {
        for (var a = this.animations[this.selectedAnimationIndex], c = a.meshObjects.length, b = 0; b < c; b++) {
            ...
            a.getWorldTransform(d.id, this.totalSeconds, d.mesh.displayMatrix, this.sceneScale, true);
            ...
            for (c = a.lightObjects.length,
                ...
                d = a.lightObjects[b],
                    d.useFixedWorldTransform || (m = this.scene.lights.getLightPos(d.lightIndex),
                        ...
                        p = Matrix.identity(),
                        a.getWorldTransform(d.id, this.totalSeconds, p, this.sceneScale, true),
                        l[0] = p[8],
                        l[1] = p[9],
                        l[2] = p[10],
                        0 != m[3] ? (m[0] = p[12],
    ...
    findLightIndexByPartIndex(a) {
        ...
        ;
    findMaterialIndexByPartIndex(a) {
    ...
    selectAnimation(a) {
        ...
        this.paused && this.setAnimationProgress(this.animationProgress, true)
    }
        ;
    selectCamera(a) {
    ...
    updateLights() {
        for (var a = this.animations[this.selectedAnimationIndex], c = this.totalSeconds * a.originalFPS, b = a.lightObjects.length, d = 0; d < b; d++) {
            var e = a.lightObjects[d]
                ...
                , k = Matrix.identity()
                , n = 1;
            e.useFixedWorldTransform || a.getWorldTransform(e.id, this.totalSeconds, k, this.sceneScale, true);
